...
class ProductView(QWidget):
    ...
    def refresh_table(self):
        ...
        for row, p in enumerate(products):
            ...
            self.table.setItem(row, 2, QTableWidgetItem(str(p.uom)))
            self.table.setItem(row, 3, QTableWidgetItem(f"{p.unit_price:.2f}"))
            self.table.setItem(row, 4, QTableWidgetItem(str(p.currency)))
    ...
    def edit_product(self):
        ...
        product = self.controller.get_by_id(product_id)
        if product:
            from src.views.product_dialog import ProductDialog
    ...
    def delete_product(self):
        row = self.table.currentRow()
        if row < 0:
            QMessageBox.warning(self, "Внимание", "Выберите изделие")
            return
    ...
    def export_to_excel(self):
        ...
        self.excel_handler.export_products(data, self)
